...
internal void
F4_Odin_ParseDeclSet(F4_Index_ParseCtx *ctx, F4_Index_Note *parent)
{
...
internal void
F4_Odin_ParseDeclSet_Braces(F4_Index_ParseCtx *ctx, F4_Index_Note *parent)
{
  if(F4_Index_RequireToken(ctx, S8Lit("{"), F4_Index_TokenSkipFlag_SkipWhitespace))
  {
    F4_Odin_ParseDeclSet(ctx, parent);
    F4_Index_RequireToken(ctx, S8Lit("}"), F4_Index_TokenSkipFlag_SkipWhitespace);
  }
}

internal void
F4_Odin_ParseDeclSet_Parens(F4_Index_ParseCtx *ctx, F4_Index_Note *parent)
{
  if(F4_Index_RequireToken(ctx, S8Lit("("), F4_Index_TokenSkipFlag_SkipWhitespace))
  {
    F4_Odin_ParseDeclSet(ctx, parent);
...
internal void
F4_Odin_SkipToOpenBraces(F4_Index_ParseCtx *ctx)
{
  for(;!ctx->done;)
  {
...
internal void
F4_Odin_ParseEnumBody(F4_Index_ParseCtx *ctx)
{
  // NOTE(fakhri): parse `enum type {` or `enum {`
...
internal F4_LANGUAGE_INDEXFILE(F4_Odin_IndexFile)
{
  ...
  for(;!ctx->done;)
  {
    ...
    if(F4_Index_RequireTokenKind(ctx, TokenBaseKind_Identifier, &name, flags))
    {
      ...
      {
        // NOTE(fakhri): type alias
        if (F4_Index_RequireToken(ctx, S8Lit("#type"), flags))
        {
          F4_Index_MakeNote(ctx, Ii64(name), F4_Index_NoteKind_Type, F4_Index_NoteFlag_ProductType);
        }
        // NOTE(fakhri): Procedures
        else if(F4_Index_RequireToken(ctx, S8Lit("proc"), flags))
        {
          F4_Index_Note *parent = F4_Index_MakeNote(ctx, Ii64(name), F4_Index_NoteKind_Function, 0);
          F4_Odin_SkipToOpenBraces(ctx);
          F4_Odin_ParseDeclSet_Parens(ctx, parent);
        }
        // NOTE(fakhri): Structs
        else if(F4_Index_RequireToken(ctx, S8Lit("struct"), flags))
        {
          ...
          F4_Odin_SkipToOpenBraces(ctx);
          F4_Odin_ParseDeclSet_Braces(ctx, parent);
        }
        // NOTE(fakhri): Unions
...
internal F4_LANGUAGE_POSCONTEXT(F4_Odin_PosContext)
{
  int count = 0;
...
internal F4_LANGUAGE_HIGHLIGHT(F4_Odin_Highlight)
{
}