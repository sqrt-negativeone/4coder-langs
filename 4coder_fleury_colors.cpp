...
#define FillFromFlag(flag) do{ t = f4_syntax_flag_transitions[BitOffset(flag)]; }while(0)
  ...
  if(ctx.token.size != 0)
  {
    ...
    switch(ctx.token.kind)
    {
      case TokenBaseKind_Identifier:
      {
        ...
        if(note)
        {
          ...
          switch(note->kind)
          {
            ...
            case F4_Index_NoteKind_Module:
            {
              FillFromFlag(F4_SyntaxFlag_Types);
              color = F4_ARGBFromID(table, fleury_color_index_rust_module);
            } break;
            
            case F4_Index_NoteKind_Namespace:
            {
              FillFromFlag(F4_SyntaxFlag_Types);
              color = F4_ARGBFromID(table, fleury_color_index_cpp_namespace);
            } break;
            
            case F4_Index_NoteKind_Trait:
            {
              FillFromFlag(F4_SyntaxFlag_Types);
              color = F4_ARGBFromID(table, fleury_color_index_rust_trait);
            }break;
      ...
      case TokenBaseKind_BuiltinFunction:
      {
        color = F4_ARGBFromID(table, fleury_color_builtin_function);
        if(!F4_ARGBIsValid(color)) { color = default_color; }
      } break;
...
#if MODAL_BUILD
      Modal_Context *modal_context = GetModalContextFromApp(app);
      switch(modal_context->current_mode)
      {
        case ModalMode_Normal:  color = F4_ARGBFromID(table, modal_cursor_normal);  break;
        case ModalMode_Insert:  color = F4_ARGBFromID(table, modal_cursor_insert);  break;
        case ModalMode_Delete:  color = F4_ARGBFromID(table, modal_cursor_delete);  break;
        case ModalMode_Yank:    color = F4_ARGBFromID(table, modal_cursor_yank);    break;
        case ModalMode_Change:  color = F4_ARGBFromID(table, modal_cursor_change);  break;
        case ModalMode_Goto:    color = F4_ARGBFromID(table, modal_cursor_goto);    break;
        case ModalMode_Search:  color = F4_ARGBFromID(table, modal_cursor_search);  break;
        case ModalMode_Comment: color = F4_ARGBFromID(table, modal_cursor_comment); break;
